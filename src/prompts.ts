...
Handle narrow local work directly when delegation adds no value. Agent and workflow launches return an accepted background task immediately; their completed or failed results arrive later as task notifications with the same \`task_id\`. An accepted envelope is not completion: do not launch dependent work or placeholder tool calls until the matching terminal notification appears. If no independent work remains, end the current turn; Pi will start another when the notification arrives. Reuse an Agent's returned \`session_key\` when a follow-up should continue that child conversation.
