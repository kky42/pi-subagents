...
- Workflow-internal \`run_agent()\` options are \`label\`, \`phase\`, \`profile\`, \`session_key\`, and \`schema\`. The profile defaults to \`general-purpose\`; a reused workflow-local session key continues the same subagent conversation. Nonfatal failures resolve to \`null\`.
...
- Schemas must use a root object. Every object must set \`additionalProperties: false\` and list every property in \`required\`; represent optional values with nullable types. \`anyOf\` is supported, but \`oneOf\` and \`allOf\` are not. Static schemas are validated before any subagent starts; dynamic schemas are validated immediately before their call.
