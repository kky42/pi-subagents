...
function assertStaticWorkflowSchemas(ast: AnyNode): void {
  ...
  const visit = (node: AnyNode): void => {
    if (node.type === "CallExpression" && node.callee?.type === "Identifier" && node.callee.name === "run_agent") {
      ...
      if (schemaNode && !(schemaNode.type === "Literal" && schemaNode.value == null)) {
        ...
        try {
          ...
          throw new Error(`Workflow schema preflight failed for run_agent()${where}: ${message}`);
        }
      }
    }
