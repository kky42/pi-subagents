...
describe("pi-subagent rendering", () => {
  ...
  function captureRenderers() {
    ...
    const mockApi: any = {
      registerTool: (tool: any) => {
        if (tool.name === "run_subagent") agentTool = tool;
      },
      registerMessageRenderer: (customType: string, renderer: unknown) => {
        if (customType === "pi-flow-task-notification-v2") notificationRenderer = renderer;
      },
  ...
  it("renders a run_subagent launch as a compact accepted task", () => {
    ...
    expect(call).toContain("Pi Subagent");
    expect(call).toContain("code-searcher");
    expect(call).toContain("Find auth files");
    expect(runningCall).toBe("");
    expect(result.trimEnd()).toBe("Subagent Find auth files accepted task_123");
  ...
  it("renders terminal custom notifications compactly and expands content", () => {
    ...
    expect(compact.trimEnd()).toBe("✓ Subagent Inspect auth task_done");
    expect(compact).not.toContain("Detailed child result");
    expect(expanded).toContain("Detailed child result");
    expect(failure).toContain("✗ Subagent Inspect tests task_failed");
