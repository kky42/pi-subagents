...
describe("pi-subagent background progress and status", () => {
  ...
  it("returns an accepted envelope without live tool updates or waiting for the child", async () => {
    ...
    const unsubscribe = session.subscribe((event) => {
      if (
        ...
        event.message.customType === "pi-flow-task-notification-v2"
      ) {
    ...
    setContextRoutingResponses(registration, async (context) => {
      if (context.tools?.some((candidate: { name?: string }) => candidate.name === "run_subagent")) {
  ...
  it.each(["tui", "rpc"] as const)("lets a %s root turn settle while its task keeps running", async (mode) => {
    ...
    setContextRoutingResponses(registration, async (context) => {
      ...
      if (!serialized.includes('"toolName":"run_subagent"')) {
        return fauxAssistantMessage([
          fauxToolCall("run_subagent", { label: "Interactive child", prompt: "Wait for release." }),
        ], { stopReason: "toolUse" });
      }
    ...
    const accepted = session.messages.find((message: any) => message.role === "toolResult" && message.toolName === "run_subagent") as any;
  ...
  it("publishes active and idle widget lines through RPC mode", async () => {
    ...
    setContextRoutingResponses(registration, async (context) => {
      ...
      if (!serialized.includes('"toolName":"run_subagent"')) {
        return fauxAssistantMessage([
          fauxToolCall("run_subagent", { label: "RPC child", prompt: "Wait for release." }),
        ], { stopReason: "toolUse" });
      }
    ...
    await waitUntil(() => widgets.some((widget) =>
      widget.lines?.some((line) => line.includes("Pi Subagent(general-purpose: RPC child)")) === true));
    childGate.resolve();
    ...
    const active = widgets.find((widget) =>
      widget.lines?.some((line) => line.includes("Pi Subagent(general-purpose: RPC child)")));
    ...
    expect(widgets.at(-1)?.lines?.[0]).toContain("pi-flow idle · 1 subagent and 0 workflows done");
    disposeSession(session);
  });
  ...
  it("steers one completion after an active root tool batch", async () => {
    ...
    const { session, registration } = await createSession({
      ...
      onWidget: (_key, lines) => {
        ...
        if (lines?.[0]?.includes("pi-flow idle · 1 subagent")) {
    ...
    setContextRoutingResponses(registration, async (context) => {
      ...
      if (!serialized.includes('"toolName":"run_subagent"')) {
        return fauxAssistantMessage([
          fauxToolCall("run_subagent", { label: "Active batch child", prompt: "Wait during the active tool batch." }),
    ...
    expect(widgets.some((lines) => lines.some((line) => line.includes("Pi Subagent(general-purpose: Active batch child)")))).toBe(true);
    expect(widgets.at(-1)).toHaveLength(1);
    expect(widgets.at(-1)?.[0]).toContain("pi-flow idle · 1 subagent and 0 workflows done");
  ...
  it("does not retry a cleared notification and persists it once", async () => {
    ...
    setContextRoutingResponses(registration, async (context) => {
      ...
      if (!serialized.includes('"toolName":"run_subagent"')) {
        return fauxAssistantMessage([
          fauxToolCall("run_subagent", { label: "Cleared child", prompt: "Wait while the root response is active." }),
        ], { stopReason: "toolUse" });
      }
    ...
    const persistedTerminals = sessionManager.getEntries().filter((entry: any) =>
      ...
      entry.customType === "pi-flow-task-notification-v2" &&
  ...
  it("consumes a post-check completion on the next root run", async () => {
    ...
    setContextRoutingResponses(registration, async (context) => {
      ...
      if (!serialized.includes('"toolName":"run_subagent"')) {
        return fauxAssistantMessage([
          fauxToolCall("run_subagent", { label: "Settling child", prompt: "Wait for agent_end." }),
        ], { stopReason: "toolUse" });
      }
      return fauxAssistantMessage("task launched");
    });
  ...
  it("steers a completion into an automatic continuation's next model context", async () => {
    ...
    setContextRoutingResponses(registration, async (context) => {
      ...
      if (!serialized.includes('"toolName":"run_subagent"')) {
        return fauxAssistantMessage([
          fauxToolCall("run_subagent", { label: "Continuation child", prompt: "Wait for tool batch." }),
        ], { stopReason: "toolUse" });
      }
      return fauxAssistantMessage("task launched");
    });
  ...
  it("keeps task completion on its originating branch during tree navigation", async () => {
    ...
    setContextRoutingResponses(registration, (context, options) => {
      ...
      if (serialized.includes("pi-flow-task-notification-v2")) {
        return fauxAssistantMessage("notification observed");
      }
      if (!serialized.includes('"toolName":"run_subagent"')) {
        return fauxAssistantMessage([
          fauxToolCall("run_subagent", { label: "Branch child", prompt: "Wait during tree navigation." }),
        ], { stopReason: "toolUse" });
      }
      return fauxAssistantMessage("task launched");
    });
  ...
  it.each(["print", "json"] as const)("keeps %s mode open until task notifications are processed", async (mode) => {
    ...
    setContextRoutingResponses(registration, async (context) => {
      ...
      if (!serialized.includes('"toolName":"run_subagent"')) {
        return fauxAssistantMessage([
          fauxToolCall("run_subagent", { label: "Print child", prompt: "Wait for release." }),
        ], { stopReason: "toolUse" });
      }
  ...
  it("persists failed terminal notifications when extension reload aborts tasks", async () => {
    ...
    setContextRoutingResponses(registration, (context, options) => {
      if (context.tools?.some((candidate: { name?: string }) => candidate.name === "run_subagent")) {
        ...
        if (!serialized.includes('"toolName":"run_subagent"')) {
          return fauxAssistantMessage([
            fauxToolCall("run_subagent", { label: "Reload child", prompt: "Wait until reload." }),
          ], { stopReason: "toolUse" });
        }
  ...
  it("starts run_subagent and run_workflow tasks after a reused extension session boundary", async () => {
    const { session, registration, model, modelRegistry, sessionManager } = await createSession({ mode: "tui" });
    ...
    const oldSubagentTool = session.getToolDefinition("run_subagent") as any;
    ...
    const terminalEntry = sessionManager.getEntries().find((entry) =>
      entry.type === "custom_message" && entry.customType === "pi-flow-task-notification-v2");
    ...
    setContextRoutingResponses(registration, (providerContext) => {
      if (providerContext.tools?.some((candidate: { name?: string }) => candidate.name === "run_subagent")) {
        return fauxAssistantMessage("notification observed");
      }
      return fauxAssistantMessage("new session child done");
    });
    const agentTool = session.getToolDefinition("run_subagent") as any;
  ...
  it("shows idle task totals and cumulative usage in the widget", async () => {
    ...
    expect(line).toContain("pi-flow idle · 1 subagent and 0 workflows done");
