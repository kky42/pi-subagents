...
- Prompt, routing, and background-lifecycle E2E review is not complete from the command exit status alone. Run the relevant driver with `--keep`, then inspect each raw root session JSONL. For every `run_agent` or `run_workflow` task, review foreground actions between its accepted result and matching terminal notification for work covered by the delegated task. Report task IDs, JSONL lines, timestamps, delegated scope, overlapping actions, and reasoning; do not count clearly non-overlapping work, lifecycle handling, or post-completion synthesis. Keep raw sessions local and remove them after review.
