...
- Do not put exact concurrency values in model-facing text. The Agent tool description states that parallel delegation is bounded and queued; the concise coordinator prompt remains routing-only.
...
- The `workflow` tool requires `name` on every call. With no script source, `name` selects a saved workflow. Inline `script` and persisted `script_path` are optional mutually exclusive sources whose `meta.name` must match `name`. `args` is still exposed to the script as the `args` global.
...
- Workflow `task_id` is also its replay and journal identity. `workflow({ name, resume_from_task_id, args })` replays the persisted script only when `name` matches the journal; adding `script_path` permits an edited script whose `meta.name` must also match. Successful cached agent results are reused for the longest unchanged prefix, then execution continues live. Cached fingerprints include prompt, label, phase, `subagent_type`, `session_key` when present, and schema; journals retain backend-native IDs for keyed continuation.
...
The appended PiFlow coordinator prompt stays concise: it keeps narrow local work in the root, states that Agent subagents and workflows run in the background and automatically notify the root when they complete or fail, states that an accepted envelope is not completion, forbids dependent work and placeholder calls until the matching terminal notification arrives, and tells the root to end the current turn when no independent work remains. It identifies Agent session-key continuation, routes complex orchestration to workflow, and asks the root to synthesize terminal results. It always includes the complete effective subagent and saved-workflow rosters without truncation. Exact invocation, workflow authoring, replay, schema, and envelope rules remain in tool descriptions and parameter schemas. Agent and workflow retain concise `promptSnippet` values for Pi's normal active-tool discovery.

`scripts/e2e/prompt-routing.mjs` is an observation-only real-model driver pinned to `openai-codex/gpt-5.6-luna` with `xhigh` thinking. It records routing, continuation, workflow shape, usage, timing, and fixture changes across scenarios without behavioral PASS/FAIL assertions; only harness or process failures make the command fail. Deterministic tests cover prompt injection and roster behavior, tool-schema shape, Agent execution, and session continuation. They do not duplicate explanatory prompt or schema prose in string assertions; review that prose directly and use observational E2E for model behavior.
