...
- Pi-backed child sessions cannot launch other pi subagents. Do not give pi child sessions the `run_agent` or `run_workflow` tool, or the flow prompt (`buildFlowPrompt`).
- External CLI backends are not given pi `run_agent`/`run_workflow` tools, but their own CLIs may expose nested/delegation features; do not try to block that from this extension.
...
- Do not put exact concurrency values in model-facing text. The run_agent tool description states that parallel delegation is bounded and queued; the concise coordinator prompt remains routing-only.
...
The appended PiFlow coordinator prompt stays concise: it keeps narrow local work in the root, states that run_agent tasks and run_workflow tasks run in the background and automatically notify the root when they complete or fail, states that an accepted envelope is not completion, forbids dependent work and placeholder calls until the matching terminal notification arrives, and tells the root to end the current turn when no independent work remains. It identifies run_agent session-key continuation, routes complex orchestration to run_workflow, and asks the root to synthesize terminal results. It always includes the complete effective subagent and saved-workflow rosters without truncation. Exact invocation, workflow authoring, replay, schema, and envelope rules remain in tool descriptions and parameter schemas. run_agent and run_workflow retain concise `promptSnippet` values for Pi's normal active-tool discovery.

`scripts/e2e/prompt-routing.mjs` is an observation-only real-model driver pinned to `openai-codex/gpt-5.6-luna` with `xhigh` thinking. It records routing, continuation, workflow shape, usage, timing, and fixture changes across scenarios without behavioral PASS/FAIL assertions; only harness or process failures make the command fail. Deterministic tests cover prompt injection and roster behavior, tool-schema shape, run_agent execution, and session continuation. They do not duplicate explanatory prompt or schema prose in string assertions; review that prose directly and use observational E2E for model behavior.
