...
## pi-flow run_subagent contract

- This repo implements a lightweight pi extension named `pi-flow`, not a fork of `refs/pi-subagents`.
- The registered tool is `run_subagent`. v2 adds an opt-in `run_workflow` tool (see "pi-flow workflows (v2)" below); the v1 contract here still governs the `run_subagent` tool.
...
- Subagent timeout is a global operator-facing guardrail (`subagentTimeoutMs` / `--subagent-timeout-ms`), not a per-call parameter on `run_subagent` or workflow `run_subagent()`. The runtime timeout is owned by `spawnSubagent` after callers acquire a concurrency slot, so queue time does not count against it.
...
- Pi-backed child sessions cannot launch other pi subagents. Do not give pi child sessions the `run_subagent` or `run_workflow` tool, or the flow prompt (`buildFlowPrompt`).
- External CLI backends are not given pi `run_subagent`/`run_workflow` tools, but their own CLIs may expose nested/delegation features; do not try to block that from this extension.
...
- Do not put exact concurrency values in model-facing text. The run_subagent tool description states that parallel delegation is bounded and queued; the concise coordinator prompt remains routing-only.
...
- Adds a second registered tool, `run_workflow`, alongside `run_subagent`: one product, two entry points. Built on the same spawn core, not a reimplementation of `refs/pi-dynamic-workflows`.
- Opt-in via `createFlowExtension({ workflow })` (compatibility alias: `createSubagentExtension`); defaults to `true`. Set `false` for a subagents-only runtime surface. The coordinator prompt deliberately does not branch on active/registered tool subsets; its static wording says "when available" for both entry points.
- The `run_workflow` tool runs a trusted, model-written JavaScript script in an isolated Worker-hosted `node:vm` context so pi can detect stalls and abort unresponsive scripts. Initial synchronous execution is bounded (5s by default), and post-`await` event-loop stalls are caught by a heartbeat watchdog. This is not a security sandbox; saved workflows are trusted code like extensions, and inline workflows are model-written code executed by the local process. Globals: `run_subagent(prompt, opts)`, `parallel(thunks)`, `pipeline(items, ...stages)`, `phase(title)`, `log(message)`, `args`, `cwd`. The script must start with `export const meta = { name, description }` (a plain literal) and call `run_subagent()` at least once.
...
- `run_subagent()` reuses the shared spawn core, so a `profile` selects a real profile and the subagent gets that profile's configured backend, model, thinking level, prompt, optional `session_key`, and (for pi-backed profiles only) tool allow-list - not stubbed guidance. `run_subagent(prompt, { schema })` requires a portable strict JSON Schema: every object must have `additionalProperties: false` and list all its properties in `required`; optional values use nullable types. Static schemas (inline object literals and top-level `const` references) are validated at script parse time before any subagent starts; dynamic options are validated at runtime before the requested agent launches. On a valid schema the subagent is forced to return one validated object: pi-backed subagents receive a terminating `structured_output` tool via `createAgentSession`'s `customTools` with the profile tool allow-list extended to admit it, while Codex-backed subagents use Codex CLI `--output-schema` and Claude-backed subagents use Claude Code `--json-schema`. The first successful structured result is captured; duplicate successful calls are ignored.
- Concurrency is the SAME global cap as `run_subagent`: both tools share one `ConcurrencyLimiter`. Normal `run_subagent` calls and workflow `run_subagent()` calls both queue and drain via `acquire`; the cap limits simultaneously running subagents, not total requested subagents. The `run_workflow` tool itself does not consume a slot; only its `run_subagent()` calls do. A workflow also has hard caps on total `run_subagent()` calls, retained logs, and orchestration-worker memory (512MB old generation by default; subagent/tool subprocess memory is not included).
- The outer `run_workflow` tool is always a background task and immediately returns a compact accepted envelope with `name` instead of run_subagent's `label`, and without `session_key`. Its script still awaits internal `run_subagent()` dependencies normally and emits one outer terminal notification. Internal subagents do not notify the foreground individually. There is no model-facing polling, steering, or scheduling. Per-call model/thinking override remains out of contract; profile-based selection via `profile` is the supported path.
- Nesting is hard-blocked for pi-backed workflow subagents: they get neither `run_subagent` nor `run_workflow`. External CLI backends use their own tool surface; this extension does not try to prevent nested/delegation features inside those CLIs.
- Do not put exact concurrency values in model-facing workflow text; the run_workflow tool description may state that fan-out is bounded and queued.
...
- `proactive-multirepo-v3`: validated two-repo parallel run_subagent fan-out under the previous routing contract.
- `proactive-fanout-v3`: validated three-lane TODO/FIXME/skipped-test run_subagent fan-out under the previous routing contract.
- `proactive-migration-v2`: validated second-opinion run_subagent delegation under the previous routing contract.
- `max-concurrent-queue`: validates `--max-concurrent-subagents 1` with two parallel normal `run_subagent` calls; both completed (`FIRST_OK`, `SECOND_OK`) and no max-concurrency rejection was emitted.
...
The appended PiFlow coordinator prompt stays concise: it keeps narrow local work in the root, states that run_subagent tasks and run_workflow tasks run in the background and automatically notify the root when they complete or fail, states that an accepted envelope is not completion, forbids dependent work and placeholder calls until the matching terminal notification arrives, and tells the root to end the current turn when no independent work remains. It identifies run_subagent session-key continuation, routes complex orchestration to run_workflow, and asks the root to synthesize terminal results. It always includes the complete effective subagent and saved-workflow rosters without truncation. Exact invocation, workflow authoring, replay, schema, and envelope rules remain in tool descriptions and parameter schemas. run_subagent and run_workflow retain concise `promptSnippet` values for Pi's normal active-tool discovery.

`scripts/e2e/prompt-routing.mjs` is an observation-only real-model driver pinned to `openai-codex/gpt-5.6-luna` with `xhigh` thinking. It records routing, continuation, workflow shape, usage, timing, and fixture changes across scenarios without behavioral PASS/FAIL assertions; only harness or process failures make the command fail. Deterministic tests cover prompt injection and roster behavior, tool-schema shape, run_subagent execution, and session continuation. They do not duplicate explanatory prompt or schema prose in string assertions; review that prose directly and use observational E2E for model behavior.
