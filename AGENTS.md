...
The appended PiFlow coordinator prompt stays concise: it keeps narrow local work in the root, explains that Agent and workflow launches return accepted background tasks, tells the root to correlate later terminal notifications by `task_id`, identifies Agent session-key continuation, routes complex orchestration to workflow, and asks the root to synthesize terminal results. It always includes the complete effective subagent and saved-workflow rosters without truncation. Exact invocation, workflow authoring, replay, schema, and envelope rules remain in tool descriptions and parameter schemas. Agent and workflow retain concise `promptSnippet` values for Pi's normal active-tool discovery.
