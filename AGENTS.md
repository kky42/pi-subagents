...
- Every top-level `run_agent` call follows Pi's normal Tool lifecycle: `execute()` remains pending until the child completes, fails, or is aborted, and then returns one terminal envelope containing `task_id`, `task_type`, `status`, `session_key`, `label`, and plain-text `content`. There is no accepted Tool result, custom completion notification, polling, steering, scheduling, per-call model override, or per-call thinking override.
- PiFlow emits versioned `pi-flow:task-state` events on Pi's synchronous event bus when a run_agent or run_workflow call starts and immediately before its Tool result returns. The payload contains only `version`, `task_id`, `task_type`, and `status`, allowing other extensions to coordinate without depending on PiFlow internals. The internal start status remains `accepted` for event compatibility and is not model-visible.
...
- The outer `run_workflow` Tool remains pending until its script and internal subagents finish, then returns one terminal envelope with its `task_id`, `name`, status, and result. Workflow-internal `run_agent()` calls keep their normal awaited, parallel, pipeline, and session-key semantics and do not produce top-level Tool results. Per-call model/thinking override remains out of contract; profile-based selection via `profile` is the supported path.
...
- Architecture: `src/core/{spawn,concurrency,model,progress,stream,task-manager}.ts` is the shared core; `src/pi-subagent.ts` owns the direct-agent Tool and extension lifecycle; `src/pi-workflow.ts` owns the Pi-facing workflow Tool and phase-tree renderer; `src/workflow/{runtime,source,structured-output,output-schema}.ts` is the workflow engine. The synchronous task manager owns active task IDs, abort/drain behavior, and coordination events, but not background scheduling or notifications.
...
- `workflow-features`: all nine real-model scenarios passed with `openai-codex/gpt-5.6-luna` at high thinking, including parallel, pipeline, structured output, branching, routing, queueing, determinism rejection, saved workflows, replay, and schema discovery. Journals correlated with terminal task IDs and replay reused cached subagent results.
- `session-key-resume`: real Pi, Codex CLI, and Claude Code backends each generated a key, resumed the same child conversation, recalled a secret unavailable from the fixture or Git, returned two correlated terminal Tool results, and emitted no custom notification.
