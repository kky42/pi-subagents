...
- The `workflow` tool now accepts exactly one source: inline `script` for ad-hoc orchestration, `name` for a saved workflow, or `script_path` for a persisted script. `args` is still exposed to the script as the `args` global.
...
- Workflow `task_id` is also its replay and journal identity. `workflow({ resume_from_task_id, args })` replays the persisted script; adding `script_path` permits an edited script. Successful cached agent results are reused for the longest unchanged prefix, then execution continues live. Cached fingerprints include prompt, label, phase, `subagent_type`, `session_key` when present, and schema; journals retain backend-native IDs for keyed continuation.
