...
- Every top-level `run_agent` call follows Pi's normal Tool lifecycle: `execute()` remains pending until the child completes, fails, or is aborted, and then returns one terminal envelope containing `task_type`, `status`, `session_key`, `label`, and plain-text `content`. There is no accepted Tool result, custom completion notification, polling, steering, scheduling, per-call model override, or per-call thinking override.
...
- Real-model E2E scripts are intentionally not part of required CI because they can be slow or inconclusive. Run `npm run e2e` before risky releases; it sequentially runs the complete real-model suite: basic metrics, workflow features, prompt routing, and session-key continuation across all backends. Use the individual `e2e:*` scripts only for focused iteration.
