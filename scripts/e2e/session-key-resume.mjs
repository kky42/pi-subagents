  ...
  for (const file of listFilesRecursive(root).filter((candidate) => candidate.endsWith(".jsonl"))) {
    for (const line of readFileSync(file, "utf8").split("\n")) {
      ...
      for (const item of Array.isArray(message?.content) ? message.content : []) {
        if (item?.type === "toolCall" && item.name === "run_subagent") calls.push(item.arguments ?? {});
      }
      if (message?.role === "toolResult" && message.toolName === "run_subagent" && message.details?.status === "accepted") {
        accepted.push(message.details);
      }
      const custom = record.type === "custom_message" ? record : message?.role === "custom" ? message : undefined;
      if (custom?.customType === "pi-flow-task-notification-v2" && custom.details?.task_type === "subagent") {
...
function writePrompt(runRoot, backend, profileName, expectedPrefix) {
  ...
  writeFileSync(promptPath, `You are testing pi-flow generated session_key continuation for backend ${backend}.\n\nYou MUST call the run_subagent tool exactly twice, sequentially, with profile "${profileName}". Do not pass session_key on the first call. Wait for its completed task notification, copy the returned session_key exactly, and pass that key on the second call. Do not invent or preselect a key.\n\nFirst run_subagent call:\n- label: "${backend} remember secret"\n- prompt exactly:\n${firstPrompt}\n\nSecond run_subagent call after the first terminal notification:\n- label: "${backend} recall secret"\n- session_key: the exact generated session_key returned by the first run_subagent task\n- prompt exactly:\n${secondPrompt}\n\nAfter the second terminal notification arrives, reply with exactly the second subagent's token line and nothing else.\n`, "utf8");
  return promptPath;
}
...
async function runBackend(options, backend) {
  ...
  try {
    ...
    assert(tasks.calls.length === 2, `[${backend}] expected exactly two run_subagent calls, observed ${tasks.calls.length}`);
    assert(tasks.calls.every((call) => call.profile === profileName), `[${backend}] run_subagent calls did not select profile ${profileName}`);
    assert(tasks.calls[0].session_key === undefined, `[${backend}] first run_subagent call must omit session_key`);
    assert(tasks.accepted.length === 2, `[${backend}] expected two accepted run_subagent results, observed ${tasks.accepted.length}`);
    assert(tasks.accepted.every((item) => item.task_type === "subagent"), `[${backend}] accepted task_type was not subagent`);
    const generatedKey = tasks.accepted[0].session_key;
    assert(/^session_[a-f0-9]{32}$/.test(generatedKey ?? ""), `[${backend}] first run_subagent did not return a generated session_key`);
    assert(tasks.calls[1].session_key === generatedKey, `[${backend}] second run_subagent call did not reuse the generated session_key`);
    assert(tasks.accepted[1].session_key === generatedKey, `[${backend}] second accepted result changed session_key`);
    assert(tasks.terminal.length === 2, `[${backend}] expected two terminal run_subagent notifications, observed ${tasks.terminal.length}`);
    assert(tasks.terminal.every((item) => item.status === "completed"), `[${backend}] a run_subagent task did not complete successfully`);
